 ...
 */
  ...
  /**
 * \ingroup lorawan
 * TracedCallback signature for tracing number of Class B Packets: 
 *
 * \param [in] deviceAddr The device address to whom the DS msg is addressed.
 * \param [in] numberofClassBTransmitted Number of Class B packets sent
 * \param [in] microSeconds Time of which a ClassBPacket was sent
 * \param [in] UID the number of Class B packets are sent (THIS ONLY IS VALID IF CLASS RX IS NOT ON)
 */

  typedef void (* LoRaWANClassBDSMessageTracedCallback) (uint32_t deviceAddr, uint32_t numberofClassBTransmitted, uint32_t microSeconds,uint64_t UID);
}  // namespace TracedValueCallback
...
typedef struct LoRaWANEndDeviceInfoNS {
  ...
  m_ClassBdownstreamTimer(), m_ClassBdownstreamTimerSchedule(), m_ClassBPingPeriodicity(1), m_ClassBChannelIndex(15), m_ClassBDataRateIndex(7), m_ClassBCodeRateIndex(1), m_downstreamTimer() {}
...
public:
  ...
  /**
   * \brief Get the maliciousDelay
   *
   * Gets the malicicous Delay in MicroSeconds 
   * The value zero means that there is no limit.
   *
   * \return the maliciousDelay in ns3:Time
   */
  uint64_t GetmaliciousDelay (void) const;


  /**
   * \brief Set the maliciousDelay
   *
   * Sets the malicicous Delay in symbols up to thousandths.
   * The value zero means that there is no delay. Symbols can be decimal amounts. 
   *
   * \param the symbols you want the beaconTime to stray from. 
   */
  void SetmaliciousDelay (double symbol); 

  /**
   * \brief Set a malcious attack time 
   *
   * Sets the malicicous attack from the start of the simulation in Microseconds 
   *
   * \param the from the start of the simulation,when should the attack happen in Microseconds
   */
  void startMaliciousAttack(uint64_t start); 
  /** 
   * \brief Get the start time of malicous attack in MicroSeconds 
   */
  uint64_t GetMaliciousAttackTime (void) const;
  void SetmaliciousDelayTimer(); 


  uint32_t GetnumberofBeacons(void) const; 
  static TypeId GetTypeId (void);
  virtual void DoInitialize (void);
  virtual void DoDispose (void);
  void PrintMDetails (void);
...
private:
  ...
  TracedCallback<uint32_t, uint8_t, Ptr<const Packet>> m_usMsgReceivedTrace;
    ...
    TracedValue <uint32_t> m_numberOfBeacons; 
    TracedCallback<uint32_t,uint32_t,uint32_t,uint64_t> m_ClassBPacketsSentTrace;            // key of device , m_nClassBPacketsSent, framecounter
    // uint64_t    m_simulationStartTime; Jayan ARiyawnasa
    bool m_offsetOn;
    Time m_maliciousDelay; 
    bool m_ismD; //is  malicious attack on 
    bool firstM; // first maliciousBeaconWasSent

    Time m_startMaliciousA; //start malicious delay
...
private:
   ...
   */
  void SendPacket ();
  Time            m_maliciousDelay; // maliciousDelay added to Beacon TimeStamp
