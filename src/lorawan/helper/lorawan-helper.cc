...
/* ... */
LoRaWANHelper::LoRaWANHelper (void) : m_deviceType (LORAWAN_DT_END_DEVICE)
{
...
LoRaWANHelper::LoRaWANHelper (bool useMultiModelSpectrumChannel) : m_deviceType (LORAWAN_DT_END_DEVICE)
{
...
LoRaWANHelper::EnableLogComponents (enum LogLevel level)
{
  ...
  LogComponentEnable ("LoRaWANdevAddress",level);
}
  ...
  for (NodeContainer::Iterator i = c.Begin (); i != c.End (); i++)
    {
      ...
      if (m_deviceType != LORAWAN_DT_GATEWAY) {
        netDevice->SetAddress (Ipv4Address(addressCounter++)); // Jayan Ariyawansa: this is where the address is created 
        // is where its implemented will also set channel on underlying phy(s)
...
NetDeviceContainer
LoRaWANHelper::Install (NodeContainer c, uint32_t NetID, int nettype)
{
  NetDeviceContainer devices;
  // static uint32_t addressCounter = 1;
   std::map<uint32_t,int> mymap;
    std::pair<std::map<uint32_t,int>::iterator,bool> ret;
  for (NodeContainer::Iterator i = c.Begin (); i != c.End (); i++)
    {
      //Map variable for storing addresses
      uint32_t devaddr; 
      devAddress addr(NetID,nettype);
     
      ret = mymap.insert ( std::pair<uint32_t,int>(addr.Get(),500) );
      ...
      if (m_deviceType != LORAWAN_DT_GATEWAY) {
        while (ret.second == false || !(addr.IsInitialized())) 
        {
          NS_LOG_INFO("DEV address already exist/failed to intialize, recalculating");
          while(!(addr.IsInitialized()))
          {
            devaddr = addr.reshuffle();
          }
          ret = mymap.insert (std::pair<uint32_t,int>(addr.Get(),500) );
        }
        netDevice->SetNetID(NetID,nettype); 
        NS_LOG_DEBUG("Setting Address: " << netDevice->GetAddress());
        netDevice->SetAddress (Ipv4Address(addr.Get())); // Jayan Ariyawansa: this is where the address is created 
        // is where its implemented will also set channel on underlying phy(s)
        netDevice->SetAttribute ("NbRep", UintegerValue (m_nbRep)); // set number of repetitions
      }
