...
LoRaWANAckTestCase::DataConfirm (LoRaWANAckTestCase *testCase, Ptr<LoRaWANNetDevice> dev, LoRaWANDataConfirmParams params)
{
  ...
  if (dev->GetAddress () == testCase->nodeAddr) 
  {
    ...
    NS_LOG_UNCOND ("DataConfirm( " << dev->GetAddress() << "): status  = " << params.m_status << " time: " << Simulator::Now());
  }
}
...
LoRaWANAckTestCase::DoRun (void)
{
  ...
  Ptr<SingleModelSpectrumChannel> uchannel = CreateObject<SingleModelSpectrumChannel> ();
  Ptr<LogDistancePropagationLossModel> upropModel = CreateObject<LogDistancePropagationLossModel> ();
  Ptr<ConstantSpeedPropagationDelayModel> udelayModel = CreateObject<ConstantSpeedPropagationDelayModel> ();
  uchannel->AddPropagationLossModel (upropModel);
  uchannel->SetPropagationDelayModel (udelayModel);

  dev0->SetChannel (uchannel);
  dev1->SetChannel (uchannel);
  /*
  Ptr<SingleModelSpectrumChannel> dchannel = CreateObject<SingleModelSpectrumChannel> ();
  Ptr<LogDistancePropagationLossModel> dpropModel = CreateObject<LogDistancePropagationLossModel> ();
  Ptr<ConstantSpeedPropagationDelayModel> ddelayModel = CreateObject<ConstantSpeedPropagationDelayModel> ();
  dchannel->AddPropagationLossModel (dpropModel);
  dchannel->SetPropagationDelayModel (ddelayModel);

  dev0->SetChannel (dchannel);
  dev1->SetChannel (dchannel);
  */
  ...
  dev0->GetMac ()->SetDataConfirmCallback (cb0); //Net Device
  ...
  params.m_loraWANChannelIndex = 15; // Since AU915 RW1 isnt using the last uplink
  params.m_loraWANDataRateIndex = 12; //RX1 DataRate is different
  params.m_loraWANCodeRate = 3;
  ...
  // takes ~78ms to send, so schedule Ack at 1s + 70ms
  Time delay = Seconds(1.07);
  if (!ackInRW1)
    delay = Seconds (3.095);
  ...
  : TestSuite ("lorawan-ack", UNIT)
{
  ...
  LogComponentEnable("LoRaWANNetDevice",LOG_LEVEL_DEBUG);
