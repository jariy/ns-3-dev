...
LoRaWANNetDevice::CompleteConfig (void)
{
  ...
  if (m_deviceType == LORAWAN_DT_END_DEVICE) {
    ...
    for (uint8_t i = 0; i < m_macs.size (); i++) {
      ...
      // Phy: set channel and data rate for listining (using SetTxConf): Jayan Ariyawansa
...
void 
LoRaWANNetDevice::SetNetID (uint32_t NetID,int nettype)
{
  NS_LOG_FUNCTION (this);
  m_mac->SetNetID (NetID,nettype);
  //Maybe a invalidator???
  
}
uint32_t LoRaWANNetDevice::GetNetID (void) const
{
  NS_LOG_FUNCTION (this);
  return m_mac->GetNetID(); 
}
int LoRaWANNetDevice::GetNettype (void) const
{
  NS_LOG_FUNCTION (this);
  return m_mac->GetNettype(); 
}
void
...
LoRaWANNetDevice::Send (Ptr<Packet> packet, const Address& dest, uint16_t protocolNumber)
{
  ...
  if (packet->RemovePacketTag (phyParamsTag)) {
    ...
    pingPeriodicity = phyParamsTag.GetClassBPingPeriodicity();
  }
  ...
  //loRaWANDataRequestParams.m_ClassBPingPeriodicity = pingPeriodicity; 
...
LoRaWANNetDevice::DataIndication (LoRaWANDataIndicationParams params, Ptr<Packet> pkt)
{
  ...
  //phyParamsTag.SetClassBPingPeriodicity(params.m_ClassBPingPeriodicity);
