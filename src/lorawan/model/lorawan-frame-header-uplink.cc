...
LoRaWANFrameHeaderUplink::LoRaWANFrameHeaderUplink () : m_devAddr((uint32_t)0), m_frameControl(0), m_frameCounter(0), m_serializeFramePort(true), m_ClassBPingPeriodicity(0)
{
}

//changed by Joe
LoRaWANFrameHeaderUplink::LoRaWANFrameHeaderUplink (Ipv4Address devAddr, bool adr, bool adrAckReq, bool ack, bool classB, uint8_t FOptsLen, uint16_t frameCounter, uint16_t framePort, uint8_t periodP) : m_devAddr(devAddr), m_frameCounter(frameCounter), m_framePort(framePort)
{
  m_frameControl = 0;
  //m_status = 0;
  ...
  setAck(ack);
  setClassB(classB);
  setClassBPingPeriodicity(periodP);
...
uint8_t
LoRaWANFrameHeaderUplink::getClassBPingPeriodicity () const
{
  return m_ClassBPingPeriodicity; 
}

void
LoRaWANFrameHeaderUplink::setClassBPingPeriodicity (uint8_t pow)
{
...
LoRaWANFrameHeaderUplink::Deserialize (Buffer::Iterator start)
{
  ...
  //Class B PingPeriodicity
  nBytes += 1; 
  uint8_t period = i.ReadU8(); 
  setClassBPingPeriodicity(period); 
