...
public:
  ...
  uint32_t GetNetID (void) const;
  int GetNettype(void) const; 
  void SetNetID (uint32_t,int);
  ...
  uint32_t m_MissedBeaconCnt; 
  uint8_t  m_initalizedB;
...
private:
  ...
  /**
   * Device's Network ID assigned by LoRa Alliance 
   */
  uint32_t NetID; 
  /** 
   * NetId type 
