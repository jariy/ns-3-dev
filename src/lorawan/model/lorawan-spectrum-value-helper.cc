...
public:
  LoRaWANSpectrumModelInitializer(void)
  {
    ...
    Bands bands;
    // The 8 default (FSB 2) 125kHz data channels available in AU915:
    // 916.8 , 917.0 ,917.2, 917.4, 917.6, 917.8 , 918.0 , 918.2 (Mhz) 
    for (int i = 0; i < 8; i++)
      {
        BandInfo bi;
        bi.fc = 916.8e6 + i * 200.0e3;
        bi.fl = bi.fc - 62.5e3; // 62.5 kHz is half of 125 kHz bandwidth
        bi.fh = bi.fc + 62.5e3;
        bands.push_back (bi);
      }

    // 9 500khz downlink channels available in AU915:
    // 923.3 923.9 924.5 925.1 925.7 926.3 926.9 927.5 923.3 (Mhz)
    for (int i = 0; i < 9; i++)
      {
        BandInfo bi;
        bi.fc = 923.3e6 + i * 600.0e3;
        bi.fl = bi.fc - 250e3; // 62.5 kHz is half of 125 kHz bandwidth
        bi.fh = bi.fc + 250e3;
        bands.push_back (bi);
      }
    /*
...
LoRaWANSpectrumValueHelper::CreateTxPowerSpectralDensity (double txPower, uint32_t freq)
{
  ...
  double txPowerDensity = txPower / (freq);
