    ...
    : m_setTRXState (), m_index (index)
{
  ...
  m_currentDataRateIndex  = 5; //  8 R8 SF12, 500KHz
  m_txPower = 2; // 2 dBm
  m_codeRate = 3;
  m_preambleLength = 8;
  m_crcOn = true;
  m_BeaconSymbolTimeout = 8;
  m_BeaconSymbolExpansionFactor = CLASSB_BEACON_SYMBOL_TO_EXPANSION_FACTOR; // 2 
...
{
  NS_LOG_DEBUG (this << (int16_t)power << (uint16_t)channelIndex << (uint16_t)dataRateIndex << (uint16_t)codeRate << (uint16_t)preambleLength << implicitHeader << crcOn);
  ...
  if (state == LORAWAN_PHY_FORCE_TRX_OFF)
    {
      if (m_trxState == LORAWAN_PHY_TRX_OFF)
        ...
        {
          NS_LOG_INFO ("force TRX_OFF, SUCCESS");
...
uint16_t LoRaWANPhy::GetBeaconSymbolTimeout(void){
  return m_BeaconSymbolTimeout; 
}
bool LoRaWANPhy::SetBeaconSymbolTimeoutExpF(uint16_t beaconSymbolTimeoutEX){
    if (beaconSymbolTimeoutEX <= 255 && beaconSymbolTimeoutEX > 0)
      {
        m_BeaconSymbolExpansionFactor = beaconSymbolTimeoutEX;
        return true; 
      }
    else
    {
      NS_LOG_WARN (" Invalid m_BeaconSymbolExpansionFactor");
      return false; 
    }
}
void
LoRaWANPhy::EnlargeWindowTimeout ()
{
  // Update beacon movement
    ...
    m_BeaconSymbolTimeout *= m_BeaconSymbolExpansionFactor; 
...
LoRaWANPhy::StartRx (Ptr<SpectrumSignalParameters> spectrumRxParams)
{
  ...
  // If the channel of the transmission and PHY don't match, just return immediatly.
  ...
  if (loraWanRxParams == 0 || dataRateMismatch)
    { // reception is not a LoRaWAN packet or is a LoRaWAN transmission with a different data rate
      //NS_LOG_DEBUG(this << "Data Rate of the tranmission don't match: transmissionDR: " << (int)loraWanRxParams->dataRateIndex << " currentDataRateIndex: " << (int)m_currentDataRateIndex);
      CheckInterference ();
  ...
  if (m_trxState == LORAWAN_PHY_RX_ON && !m_setTRXState.IsRunning ())
    {
      ...
      //const LoRaSpreadingFactor sf = LoRaWAN:: [transmissionDataRateIndex].spreadingFactor;
      ...
      *interferenceAndNoise += *m_noise;
      ...
      if (sinr_db > sinr_cutoff_db)
        ...
        {
          NS_LOG_DEBUG("LORAWAN_RX_DROP_SINR_TOO_LOW");
  ...
  else
    {
      ...
      NS_LOG_DEBUG (this << " Packet Dropped: transceiver not in RX state (state = " << m_trxState << ")");
          ...
          if (m_random->GetValue () < per)
            {
              ...
              m_currentRxPacket.second.destroyed = false; //true
              NS_LOG_WARN("No more Error Model");
            }
        }
      else
        {
  ...
  if (currentRxParams == params)
    {
      ...
      if (!m_currentRxPacket.second.destroyed && !m_currentRxPacket.second.aborted)
        ...
        {
          NS_LOG_DEBUG(this <<" Packet was destroyed at : " << Simulator::Now() );
...
LoRaWANPhy::CalculateTxTime (uint8_t payloadLength)
{
  ...
  NS_LOG_DEBUG(this << ": sf: " << sf  << "| codeRate:" << (uint16_t)m_codeRate  << "| payloadlength:" << (uint16_t)payloadLength
      << "| preamblelength: " << (uint16_t) m_preambleLength  << "| SymbolPayload:" << nSymbolsPayload  << "| ConditionalSymbolsPayload:" << nConditionalSymbolsPayload
      << "| time:" << txTime << "uS" << "implicitHeader:" << implicitHeader);
...
Time
LoRaWANPhy::CalculatePreambleTime (uint16_t missedBeacons)
{
  const uint32_t bandwidth = LoRaWAN::m_supportedChannels [m_currentChannelIndex].m_bw;
  const LoRaSpreadingFactor sf = LoRaWAN::m_supportedDataRates [m_currentDataRateIndex].spreadingFactor;
  ...
  if (missedBeacons != 0)
  {
    NS_LOG_WARN(this << "Missed Beacon");
    nSymbolsPreamble = m_BeaconSymbolTimeout;
  }
  NS_LOG_INFO(this << "Beacon nSymbolsPreamble: " << nSymbolsPreamble);
  double timePreamble = nSymbolsPreamble * symbolPeriod;
  return MicroSeconds(timePreamble);
}
