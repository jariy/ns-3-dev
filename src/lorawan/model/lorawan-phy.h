 ...
 */
 ...
 /*!
   * \param [IN] Number of Missed Beacons.
   * \brief Calculate the time for transmittin a preamble in microseconds with Beaconless Mode Option
   * Note number of preamble symbols is supposed to be already configured in the PHY
   * \retval Returns the symbol time in ns3:Time (microseconds)
  */
  Time CalculatePreambleTime (uint16_t missedBeacons);

  /**
   * Check whether PHY has detected a premable since it switched its state to RX_ON
   */
  bool preambleDetected (void) const;
  uint16_t GetBeaconSymbolTimeout();
  ...
  /*
  * Enlarge the beaconWindow/ Ping Windowby increasing both m_pingSlotTimeout and m_BeaconSymbolTimeout and m_BeaconWindowMovement
  */

  void EnlargeWindowTimeout (void); 

  /*
  * Sets the m_BeaconSymbolExpansionFactor; 
  * @param beaconSymbolTimeoutEx Symbol Expansion rate
...
private:
  /* 
  * Beacon Symbol for Expansion
  *
  */
  uint16_t m_BeaconSymbolExpansionFactor; 
  /**
