...
namespace ns3 {
  ...
  LoRaWANNetworkServer::GetTypeId (void)
  {
    ...
    BooleanValue(true), 
    MakeBooleanAccessor (&LoRaWANNetworkServer::m_offsetOn),
    MakeBooleanChecker())
    ...
    .AddAttribute ("ClassBDownstream", "A RandomVariableStream used to pick the time between subsequent Class B DS transmissions to an end device.",
     ...
     MakePointerChecker <RandomVariableStream>())
    .AddAttribute("maliciousDelaySymbol", "Number of Symbols to set Malicious Delay on BeaconTime",
     DoubleValue (0.0),
     MakeDoubleAccessor (&LoRaWANNetworkServer::GetmaliciousDelay,
      &LoRaWANNetworkServer::SetmaliciousDelay),
      MakeDoubleChecker<double> (0, 9999))
    .AddAttribute("startMaliciousAttack", 
      "In Microseconds, when should we start the beacon spoofing.(Microseconds)" 
      "O Means no attack",
      UintegerValue(0.0),
      MakeUintegerAccessor(&LoRaWANNetworkServer::GetMaliciousAttackTime,
      &LoRaWANNetworkServer::startMaliciousAttack),
    ...
    .AddTraceSource ("USMsgReceived",
     ...
     "ns3::TracedValueCallback::LoRaWANDSMessageTracedCallback")
     .AddTraceSource ("numberofBeacons",
     "Number of Beacons Sent",
     MakeTraceSourceAccessor (&LoRaWANNetworkServer::m_numberOfBeacons),
     "ns3::TracedValueCallback::Uint32")
    .AddTraceSource ("ClassBPacketsSentTrace",
     "Number of Class B packets sent",
     MakeTraceSourceAccessor (&LoRaWANNetworkServer::m_ClassBPacketsSentTrace),
     "ns3::TracedValueCallback::LoRaWANClassBDSMessageTracedCallback")
    ;
    return tid;
  }
  ...
  //begin broadcasting of beacons
    ...
    Time BeaconDelay = MicroSeconds(1500); // Allows for gateway to change from receive to transmit mode (LoRaWAN® L2 1.0.4 Specification)
    Time Accuracy = MicroSeconds(1); // 
    Time t = Seconds (128); //+ BeaconDelay; 
      m_beaconTimer = Simulator::Schedule (t, &LoRaWANNetworkServer::ClassBSendBeacon, this);
      NS_LOG_DEBUG (this << "GW: Class B beacon " << "scheduled at " << t);  
    }  
  ...
  // Populate m_endDevices based on ns3::NodeList
    for (NodeList::Iterator it = NodeList::Begin (); it != NodeList::End (); ++it)
    {
      ...
      if (ipv4DevAddr == Ipv4Address(0xffffffff)) { // gateway? Jayan Ariyawansa
        continue;
      }
      ...
      NS_LOG_ERROR (this << "GW: Unable to allocate device address");
      continue;
    }
  }
...
LoRaWANNetworkServer::InitEndDeviceInfo (Ipv4Address ipv4DevAddr)
{
  ...
  if (m_generateDataDown) {
    ...
    NS_LOG_DEBUG (this << "GW: DS Traffic Timer for node " << ipv4DevAddr << " scheduled at " << t);
  }
...
LoRaWANNetworkServer::HandleUSPacket (Ptr<LoRaWANGatewayApplication> lastGW, Address from, Ptr<Packet> packet)
{
  ...
  if (it == m_endDevices.end ()) { // not found, so create a new struct and insert it (note this should have already happened in DoInitialize()):
    NS_LOG_WARN (this << "GW: end device with address = " << deviceAddr << " not found in m_endDevices, allocating");
  ...
  if (frmHdr.getFrameCounter () <= it->second.m_fCntUp && !firstRX) {
    ...
    if (t <= Seconds (1.0)) { // assume US packet is really a duplicate received by a second gateway
      ...
      NS_LOG_INFO (this << "GW: Duplicate detected: " << frmHdr.getFrameCounter () << " <= " << it->second.m_fCntUp << " &&  t = " << t << " < 1 second => dropping packet");
  ...
  if (packet->RemovePacketTag (phyParamsTag)) {
    ...
    if(it->second.m_isClassB == true)
    {
      it->second.m_ClassBPingPeriodicity = frmHdr.getClassBPingPeriodicity();
      it->second.m_ClassBPingSlots = std::pow(2.0, it->second.m_ClassBPingPeriodicity);
      NS_LOG_DEBUG(it->second.m_deviceAddress.Get()<< " 's ClassBPingSlots: " << (int)it->second.m_ClassBPingSlots);
    }
  } else {
    NS_LOG_WARN (this << " GW: LoRaWANPhyParamsTag not found on packet.");
  }
  ...
  if (packet->RemovePacketTag (msgTypeTag)) {
    ...
    if (msgType == LORAWAN_CONFIRMED_DATA_UP) {
      ...
      NS_LOG_DEBUG (this << "GW: Received Confirmed Data UP. Next DS Packet will have Ack bit set");
    }
  } else {
    NS_LOG_WARN (this << "GW: LoRaWANMsgTypeTag not found on packet.");
  }
  ...
  if (processMACAck && frmHdr.getAck ()) {
    ...
    if (!it->second.m_downstreamQueue.empty ()) { // there is a DS message in the queue
      if (it->second.m_downstreamQueue.front()->m_downstreamMsgType == LORAWAN_CONFIRMED_DATA_DOWN) { // End device confirmed reception of DS packet, so we can remove it:
        ...
        NS_LOG_DEBUG (this << "GW: Received Ack for Confirmed DS packet, removing packet from DS queue for end device " << deviceAddr);
      } else {
        NS_LOG_ERROR (this << "GW: Upstream frame has Ack bit set, but downstream frame msg type is not Confirmed (msgType = " << it->second.m_downstreamQueue.front()->m_downstreamMsgType << ")");
      }
    } else {
      ...
      NS_LOG_WARN (this << "GW: Upstream frame has Ack bit set, but there is no downstream frame queued.");
    }
  }
  if(it->second.m_isClassB == true)
  {
    //it->second.m_ClassBPingPeriodicity = phyParamsTag.GetClassBPingPeriodicity();
  ...
  if (frmHdr.getClassB () && !(it->second.m_isClassB) && m_generateClassBDataDown) {
    ...
    NS_LOG_DEBUG("GW: Turning on Class B mode");
    it->second.m_isClassB = true;
    it->second.m_ClassBPingPeriodicity = frmHdr.getClassBPingPeriodicity();
    it->second.m_ClassBPingSlots = std::pow(2.0, it->second.m_ClassBPingPeriodicity); // number of ping slots is based on ping periodicity, and is always a power of two.
    ...
    Time t = Seconds(this->m_ClassBdownstreamRandomVariable->GetValue ()/(it->second.m_ClassBPingPeriodicity+1));
    it->second.m_ClassBdownstreamTimer = Simulator::Schedule (t, &LoRaWANNetworkServer::ClassBDSTimerExpired, this, key);
    NS_LOG_INFO (this << "GW: Class B DS Traffic Timer for node " << it->second.m_deviceAddress << " scheduled at " << t);

    Time t2 = Seconds(this->m_ClassBdownstreamIATRandomVariable->GetValue ()/(it->second.m_ClassBPingPeriodicity+1));
    it->second.m_ClassBdownstreamTimerSchedule = Simulator::Schedule (t2, &LoRaWANNetworkServer::ClassBScheduleExpiry, this, key);
    //NS_LOG_WARN (this << "GW: Class B DS Traffic Timer for node " << it->second.m_deviceAddress << " scheduled at " << t2);
    
  } else if (!(frmHdr.getClassB ()) && it->second.m_isClassB && m_generateClassBDataDown) {
    //turning off Class B mode
    NS_LOG_DEBUG("GW: Turning off Class B mode");
  ...
  if (it->second.m_rw1Timer.IsRunning()) {
    NS_LOG_ERROR (this << "GW: Scheduling RW1 timer while RW1 timer was already scheduled for " << it->second.m_rw1Timer.GetTs ());
  }
...
LoRaWANNetworkServer::RW1TimerExpired (uint32_t deviceAddr)
{
  ...
  bool foundGW = false;
  // The RW1 LoRa channel and data rate are the same as used in the last US transmission + 8 AU915 Jayan Ariyawansa
  const uint8_t dsChannelIndex = 7 + it_ed->second.m_lastChannelIndex % 8;
  ...
  if (!foundGW) {
    NS_LOG_DEBUG (this << "GW: No gateway available for transmission in RW1, scheduling timer for DS transmission in RW2");
    ...
    if (it_ed->second.m_rw2Timer.IsRunning()) {
      NS_LOG_ERROR (this << "GW: Scheduling RW2 timer while RW2 timer was already scheduled for " << it_ed->second.m_rw2Timer.GetTs ());
    }
    ...
    NS_ASSERT (receiveDelay.GetMicroSeconds() > 0); //Jayan Ariyawansa
...
LoRaWANNetworkServer::RW2TimerExpired (uint32_t deviceAddr)
{
  ...
  if (!foundGW) {
    ...
    if (HaveSomethingToSendToEndDevice (deviceAddr)) {
      m_nrRW2Missed++;
      NS_LOG_INFO (this << "GW: Unable to send DS transmission to device addr " << deviceAddr << " in RW1 and RW2, no gateway was available.");
    }
  }
}
...
LoRaWANNetworkServer::SendDSPacket (uint32_t deviceAddr, Ptr<LoRaWANGatewayApplication> gatewayPtr, bool RW1, bool RW2)
{
  ...
  if (it == m_endDevices.end ()) { // end device not found
    NS_LOG_ERROR (this << "GW: Could not find device info struct in m_endDevices for dev addr " << deviceAddr << ". Aborting DS Transmission");
    return;
  }
  ...
  if (it->second.m_downstreamQueue.size() > 0) {
    ...
    if (!it->second.m_setAck) {
      ...
      NS_LOG_INFO (this << "GW: No downstream packet found nor is ack bit set for dev addr " << deviceAddr << ". Aborting DS transmission");
      return;
    } else {
      NS_LOG_DEBUG (this << "GW: Generating empty downstream packet to send Ack for dev addr " << deviceAddr);
  ...
  if (RW1) {
    dsChannelIndex = 7 + it->second.m_lastChannelIndex % 8;
    ...
    NS_FATAL_ERROR (this << "GW: Either RW1 or RW2 should be true");
    return;
  }
  ...
  NS_LOG_DEBUG (this << "GW: Sent DS Packet to device addr " << deviceAddr << " via GW #" << gatewayPtr->GetNode()->GetId() << " in RW" << (RW1 ? "1" : "2"));
...
LoRaWANNetworkServer::DSTimerExpired (uint32_t deviceAddr)
{
  ...
  auto it = m_endDevices.find (deviceAddr);
  if (it == m_endDevices.end ()) { // end device not found
    NS_LOG_ERROR (this << "GW: Could not find device info struct in m_endDevices for dev addr " << deviceAddr);
    return;
  }

  // Generate a Downstream packet
  if (it->second.m_downstreamQueue.size () > 0)
    NS_LOG_INFO(this << "GW: DS queue for end device " << Ipv4Address(deviceAddr) << " is not empty");

  NS_ASSERT (m_pktSize >= 8 + 1 + 4); // should be able to send at least frame header, MAC header and MAC MIC
  bool generatePacket = true;
  if (m_pktSize < 8 + 1 + 4) {
    NS_LOG_ERROR (this << "GW: m_pktSize = " << m_pktSize << " is too small, not generating DS packets");
  ...
  if (generatePacket) {
    ...
    NS_LOG_INFO (this << "GW: Added downstream packet with size " << m_pktSize  << " to DS queue for end device " << Ipv4Address(deviceAddr) << ". queue size = " << (int)it->second.m_downstreamQueue.size());
  }

  // Reschedule timer:
  Time t = Seconds(this->m_downstreamIATRandomVariable->GetValue ());
  it->second.m_downstreamTimer = Simulator::Schedule (t, &LoRaWANNetworkServer::DSTimerExpired, this, deviceAddr);
  NS_LOG_DEBUG (this << "GW: DS Traffic Timer for end device " << it->second.m_deviceAddress << " scheduled at " << t);
}
...
LoRaWANNetworkServer::DeleteFirstDSQueueElement (uint32_t deviceAddr)
{
  auto it = m_endDevices.find (deviceAddr);
  if (it == m_endDevices.end ()) { // end device not found
    NS_LOG_ERROR (this << "GW: Could not find device info struct in m_endDevices for dev addr " << deviceAddr << ". Unable to delete DS queue element.");
    return;
  }
...
uint64_t 
LoRaWANNetworkServer::GetmaliciousDelay (void) const 
{
  return (uint64_t)m_maliciousDelay.GetMicroSeconds();
}
void 
LoRaWANNetworkServer::SetmaliciousDelay (double secs)
{
  //const uint32_t bandwidth = LoRaWAN::m_supportedChannels [m_ClassBBeaconChannelIndex].m_bw;
  //const LoRaSpreadingFactor sf = LoRaWAN::m_supportedDataRates [m_ClassBBeaconDataRateIndex].spreadingFactor;
  //double symbolRate = ((double)bandwidth)/pow(2.0, sf);
  //double symbolPeriod = 1.0e6/symbolRate; // the symbol period in microseconds
  m_maliciousDelay = Seconds(secs);
  NS_LOG_WARN("Inputing malicious BeaconTime" << m_maliciousDelay.GetMilliSeconds());
}
void
LoRaWANNetworkServer::SetmaliciousDelayTimer()
{
  m_ismD = true;
  NS_LOG_WARN("Starting Malicous Beacon Attack NOW with beaconTimeDelay: " << (uint64_t)m_maliciousDelay.GetMicroSeconds() << "us "<< Simulator::Now());
}
void 
LoRaWANNetworkServer::startMaliciousAttack(uint64_t start) {
    if (start != 0)
    {
    m_startMaliciousA = Seconds(start); 
    Simulator::Schedule(m_startMaliciousA,&LoRaWANNetworkServer::SetmaliciousDelayTimer,this);
    }
}

uint64_t 
LoRaWANNetworkServer::GetMaliciousAttackTime (void) const
{
  return (uint64_t)m_startMaliciousA.GetMicroSeconds();
}
void
LoRaWANNetworkServer::ClassBScheduleExpiry(uint32_t deviceAddr)
{
    ...
    NS_LOG_INFO (this << "GW: Class B DS Traffic Timer for node " << deviceAddr << " scheduled at " << t);
...
uint32_t 
LoRaWANNetworkServer::GetnumberofBeacons(void) const 
{
  return m_numberOfBeacons; 
}
void
LoRaWANNetworkServer::ClassBDSTimerExpired (uint32_t deviceAddr)
{
  ...
  if (it->second.m_ClassBdownstreamQueue.size () > 0)
    NS_LOG_INFO(this << "GW: Class B DS queue for end device " << Ipv4Address(deviceAddr) << " is not empty");
  ...
  if (m_ClassBpktSize < 8 + 1 + 4) {
    NS_LOG_ERROR (this << "GW: m_pktSize = " << m_pktSize << " is too small, not generating Class B DS packets");
    generatePacket = false;
  }
  ...
  if (generatePacket) {
    ...
    m_dsMsgGeneratedTrace (deviceAddr, element->m_downstreamTransmissionsRemaining, element->m_downstreamMsgType, element->m_downstreamPacket);
    NS_LOG_INFO (this << "GW: Added downstream packet with size " << m_pktSize  << " to DS queue for end device " << Ipv4Address(deviceAddr) << ". queue size = " << it->second.m_downstreamQueue.size());
  }
}
...
LoRaWANNetworkServer::ClassBSendBeacon (){
  ...
  /*TODO: Jayan Ariyawansa fix encryption
  ...
  */
  NS_LOG_FUNCTION (this << "GW: attempt to send beacon at" << Simulator::Now ().GetSeconds () );
  ...
  if (m_ismD == true)
  {
    if (m_maliciousDelay.GetNanoSeconds() != 0)
    {
      NS_LOG_DEBUG("Gateway is now sending malicious Beacons m_maliciousDelay: " << m_maliciousDelay.GetSeconds());
    }
    ftimestamp = Simulator::Now() + m_maliciousDelay;
  }
  else
  {
    ftimestamp = Simulator::Now(); //+ BeaconDelay;
  }
   //Time BeaconDelay = MilliSeconds((uint64_t)1.5); // Allows for gateway to change from receive to transmit mode (LoRaWAN® L2 1.0.4 Specification)
  Time Accuracy = MicroSeconds(1); 
  Time nextBeacon = Seconds (128); //+ BeaconDelay; 
  

  m_numberOfBeacons++;

  uint64_t secs = ftimestamp.GetMilliSeconds() / 1000; //note: not using GetSeconds as it returns a double.
  uint64_t t = secs & 0x00000000FFFFFFFF; // t should be the 4 LSB bytes of secs TODO: there must be a nicer way of doing this.

  NS_LOG_DEBUG("GW: Current time is " << timestamp);
  NS_LOG_DEBUG("GW: Next beacon will be sent at " << nextBeacon + timestamp); //- BeaconDelay);
  ...
  NS_LOG_DEBUG("GW: time put into beacon: " << time);
  ...
  //Simulator::ScheduleNow(&LoRaWANGatewayApplication::SendBeacon,*gw,p)
  ...
  for (auto gw = m_gateways.cbegin(); gw != m_gateways.cend(); gw++) {
    if ((*gw)->CanSendImmediatelyOnChannel (m_ClassBBeaconChannelIndex, m_ClassBBeaconDataRateIndex)) {
        ...
        else if (m_ismD == 1)
        {
          ...
          {
            Simulator::Schedule(m_maliciousDelay,&LoRaWANGatewayApplication::SendBeacon,*gw,p);
          }
        ...
        NS_LOG_WARN ("GW: LoRaWANNetworkServer: a gateway was unable to send a beacon"); //TODO: log which one
    }
  }
  if (m_ismD == 1 && firstM == 0)
  {
    firstM =1; 
  }
  ...
  for (auto d = m_endDevices.cbegin(); d != m_endDevices.cend(); d++) {
    ...
    */
    ...
    if(d->second.m_isClassB) {
      ...
      uint8_t key[16] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 }; // crypto not implemented across layers yet, assuming key of all 0s. Jayan Ariyawansa   
      uint8_t buf[16] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };

      double secs = timestamp.GetSeconds(); // convert to a 4 byte buffer NEED GMT TIME FOR SURE
      ...
      if (m_offsetOn == 1){
        ...
        //NS_LOG_WARN (" GWO: " << O << (char)buf[0] << " buf[1]: "<< (char)buf[1]);
      }
      else 
      {
        O = 0; 
      ...
      NS_LOG_DEBUG("GW: Scheduling ping slots for device" << deviceAddr.Get ());
      for(uint i=0;i< d->second.m_ClassBPingSlots; i++){
        ...
        //NS_LOG_WARN("GW PingSlot Offset Slot: " << O + period*i);
...
LoRaWANNetworkServer::ClassBPingSlot(uint32_t devAddr, uint64_t pingTime)
{
  ...
  if (it == m_endDevices.end ()) { // end device not found
    NS_LOG_ERROR (this << "GW: Could not find device info struct in m_endDevices for dev addr " << devAddr);
    return;
  }
  ...
  if (it->second.m_ClassBdownstreamQueue.size() > 0) 
  {
      ...
      NS_LOG_DEBUG (this << "GW: No downstream packet to send to: " << devAddr << ". Aborting DS transmission");
      return;
  } 
  ...
  {   
      ...
      {
          //log err
          NS_LOG_DEBUG (this << "GW: Ping slot can't be used because of duty cycle limits. Potential packet to " << devAddr << " not sent. Aborting DS transmission");
      ...
      m_ClassBPacketsSentTrace(devAddr,it->second.m_nClassBPacketsSent,Simulator::Now().GetMilliSeconds(),p->GetUid()); 

      // Ask gateway application to send the DS packet:
      
      //NS_LOG_WARN("GW sending Packet with info: " << temp );
      NS_LOG_DEBUG("GW: Sending a downlink ping, from " << (*gw)->GetNode ()->GetDevice (0)->GetAddress () << " to " << Ipv4Address (devAddr) << "at time " << Simulator::Now() );
  ...
  {
      //log err
      NS_LOG_DEBUG (this << "GW: Ping slot overlap. Potential packet to " << devAddr << " not sent. Aborting DS transmission");
      return;
  }
}
...
{
  for (auto d = m_endDevices.cbegin(); d != m_endDevices.cend(); d++) {
    std::cout << d->second.m_deviceAddress.Get() << "\t m_nDSPacketsGenerated:" <<  d->second.m_nDSPacketsGenerated <<  
    "\t m_nDSPacketsSent:" << d->second.m_nDSPacketsSent << "\t m_nDSPacketsSentRW1:" << d->second.m_nDSPacketsSentRW1 << "\t m_nDSPacketsSentRW2:" << d->second.m_nDSPacketsSentRW2 << 
    "\t m_nDSRetransmission:" << d->second.m_nDSRetransmission << "\t m_nDSAcks:" << d->second.m_nDSAcks << "\t m_numberOfBeacons:" << m_numberOfBeacons << 
    "\t m_nClassBPacketsGenerated:" << d->second.m_nClassBPacketsGenerated << "\t m_nClassBPacketsSent:" << d->second.m_nClassBPacketsSent << "\t m_nUSPackets:" << d->second.m_nUSPackets << std::endl;
  }
  
}
void
LoRaWANNetworkServer::PrintMDetails ()
{
...
LoRaWANGatewayApplication::CanSendImmediatelyOnChannel (uint8_t channelIndex, uint8_t dataRateIndex)
{
  ...
  if (!device) {
    NS_LOG_ERROR (this << "GW: Cannot get LoRaWANNetDevice pointer belonging to this gateway");
    return false;
  } else {
...
void LoRaWANGatewayApplication::SendDSPacket (Ptr<Packet> p)
{
  ...
  uint8_t dataRateIndex = 7; // SF12 as default value
  ...
  NS_LOG_DEBUG ("GW: At time " << Simulator::Now ().GetSeconds () // INFO
...
void LoRaWANGatewayApplication::StopApplication () // Called at time specified by Stop
{
  ...
  {
    NS_LOG_WARN ("GW: LoRaWANGatewayApplication found null socket to close in StopApplication");
  }
}
...
void LoRaWANGatewayApplication::HandleRead (Ptr<Socket> socket)
{
  ...
  while ((packet = socket->RecvFrom (from)))
  {
    if (packet->GetSize () == 0)
        ...
        {
          NS_LOG_INFO ("GW: At time " << Simulator::Now ().GetSeconds ()
           << "s gateway on node #"
           << GetNode()->GetId()
...
LoRaWANGatewayApplication::SendBeacon (Ptr<Packet> packet)
{
  ...
  uint8_t beaconChannelIndex = 15; // 927.5 MHz. Mandetory for Class B beacons. CHANGED 
  uint8_t beaconDataRateIndex = 7;  // SF12, 500kHz BW. Mandetory for Class B beacons.
  uint8_t beaconCodeRate = 1; 
  uint8_t beaconPreambleLength = 8; // 8 symbols for the preamble in beacon frames !@S@##$: Issue 
  ...
  NS_LOG_DEBUG("GW: sending the beacon." << Simulator::Now());
  this->SendDSPacket (p);
}
...
LoRaWANGatewayApplication::SetDefaultClassBDataRateIndex (uint8_t index)
{
  ...
  if (index <= LoRaWAN::m_supportedDataRates.size () - 1 && index >= 7)
    ...
    NS_LOG_ERROR (this << "GW: " << index << " is an invalid data rate index");
}
