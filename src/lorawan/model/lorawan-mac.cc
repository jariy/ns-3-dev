...
LoRaWANMac::SetRX1DROffset (uint8_t offset)
{
  ...
  uint32_t LoRaWANMac::GetNetID (void) const
  {
    return NetID; 
  }
  int LoRaWANMac::GetNettype(void) const
  {
    return nettype;
  }
  void LoRaWANMac::SetNetID (uint32_t ID,int type)
  {
    NetID = ID;  
    nettype = type; 
  }
...
LoRaWANMac::SetLoRaWANMacState (LoRaWANMacState macState)
{
  ...
  if (macState == MAC_IDLE) {
      ...
      NS_LOG_DEBUG (this << " Switching to MAC_RW1: " << Simulator::Now());
      ...
      if (receiveDelay.GetMicroSeconds() >= 0)
      ...
      NS_LOG_DEBUG (this << " Switching to MAC_RW2: " << Simulator::Now());
    ...
    if(m_LoRaWANMacState == MAC_IDLE) {
      ...
      NS_LOG_DEBUG(this << "device is busy during scheduled beacon receive; slot missed. LoRaWANMacState: " << m_LoRaWANMacState);
    }
...
LoRaWANMac::PdDataIndication (uint32_t phyPayloadLength, Ptr<Packet> p, uint8_t lqi, uint8_t channelIndex, uint8_t dataRateIndex, uint8_t codeRate)
{
  ...
  if(macHdr.getLoRaWANMsgType() == 0){  //TODO: the MAC header is not included in the beacon frame. As the Join procedure is not yet modeled in the simulator this will work as a way of identifying beacon frames.
    ...
    params.m_MIC = 0; // no MIC in beacon
     if (m_LoRaWANMacState == MAC_BEACON ) // just to make the end device in the beacon_reserved mode. 
    {
      m_MissedBeaconCnt = 0;  
      m_initalizedB = 1; //Class B is init
    }
    ...
    if (acceptFrame) {
        ...
        if (frameHdr.IsAck ()) { // process Ack for Class A device
          if (m_txPkt != 0) {
            ...
            { // Call callback, informing succesfull delivery of frame
              NS_LOG_DEBUG ("PdDataIndication ():  End Device: Packet is for me; forwarding up");
      ...
      if (!m_dataIndicationCallback.IsNull ())
      {
        NS_LOG_DEBUG ("PdDataIndication ():  Gateway: Packet is for me; forwarding up");
        m_dataIndicationCallback (params, pktCopy);
      }
    } else {
      m_macRxDropTrace (p);
...
LoRaWANMac::SetTRXStateConfirm (LoRaWANPhyEnumeration status)
{
  ...
  if (m_LoRaWANMacState == MAC_IDLE)
  ...
  else if (m_LoRaWANMacState == MAC_TX && status == LORAWAN_PHY_TX_ON)
    {
      ...
      NS_LOG_DEBUG("airtime required is " << airTime);
...
LoRaWANMac::CheckQueue ()
{
  ...
  NS_LOG_INFO (this << " INFO: tx queue size is equal to " << m_txQueue.size());
  ...
  if (m_LoRaWANMacState == MAC_IDLE && !m_txQueue.empty () && m_txPkt == 0 && !m_setMacState.IsRunning ())
  {
    ...
    if (m_txQueue.empty ()) {
      NS_LOG_INFO (this << " tx queue is empty, so there is no packet to send.");
    }
    if (m_txPkt) {
...
LoRaWANMac::OpenRW ()
{
  ...
  if (m_LoRaWANMacState == MAC_RW1) {
    // RW1 uses the same channel as the preceding uplink Jayan Ariyawansa CHANGED to transmit % 8 
    // The data rate is a function of the uplink data rate and the RX1DROffset
    uint8_t channelIndex = (LoRaWAN::m_supportedChannels.size()-10 ) + (8 -  m_phy->GetCurrentChannelIndex () % 8) ;
    NS_LOG_INFO (this << " MAC_RW1 Channel Number: " << (int)channelIndex );
    uint8_t dataRateIndex = LoRaWAN::GetRX1DataRateIndex (m_phy->GetCurrentDataRateIndex (), m_RX1DROffset);
    NS_LOG_INFO (this << " MAC_RW1 DataRate Index: " << (int)dataRateIndex );
    ...
    // The default fixed RW2 channel is 927.5 MHz / DR8 (SF12, 500kHz)
    ...
    uint8_t channelIndex = 15; // 927.5MHz. Mandetory for Class B beacons. TODO: no magic numbers
    uint8_t dataRateIndex = 7; // SF12, 500kHz BW. Mandetory for Class B beacons.
    ...
    if (!m_phy->SetTxConf (maxTxPower, channelIndex, dataRateIndex, codeRate, 8, true, false) ) {
      NS_LOG_ERROR (this << " unable to configure Phy");
      return;
    } 
    if (m_MissedBeaconCnt != 0)
    {
      m_phy->EnlargeWindowTimeout();
    }
    else
    {
      m_phy->ResetWindowTimeout(); 
    }
    m_phy->SetTRXStateRequest (LORAWAN_PHY_RX_ON);
    Time preambleTime = m_phy->CalculatePreambleTime (m_MissedBeaconCnt);
    if (m_initalizedB == 0)
    {
      preambleTime = MilliSeconds(2120); 
    }
    NS_LOG_WARN(this <<" Beacon PreambleTime: " << preambleTime.GetSeconds());
    ...
    //NS_LOG_WARN("PING SLOT PREAMBLE: " << m_phy->CalculatePreambleTime ().GetSeconds());
  }
  else {
  ...
  if (m_LoRaWANMacState == MAC_RW1) { // no frame received, so should continue to RW2
    ...
    if (confirmed) {
      ...
      if (m_ackTimeOut.IsRunning ()) {
        ...
        NS_LOG_INFO (this << " Closing RW2 after Ack Timeout timer has expired. Skipping MAC_ACK_TIMEOUT state and going directly to MAC_IDLE state.");
        m_setMacState = Simulator::ScheduleNow (&LoRaWANMac::SetLoRaWANMacState, this, MAC_IDLE);
      }
    } else {
      m_setMacState = Simulator::ScheduleNow (&LoRaWANMac::SetLoRaWANMacState, this, MAC_IDLE);
    ...
    NS_LOG_INFO (this << " Missed beacon. Going directly to MAC_IDLE state.");
    m_setMacState = Simulator::ScheduleNow (&LoRaWANMac::SetLoRaWANMacState, this, MAC_IDLE);
  }
  else if (m_LoRaWANMacState == MAC_CLASS_B_PACKET) {
    NS_LOG_INFO (this << " No packet received in this ping slot. Going directly to MAC_IDLE state." << Simulator::Now());
    m_setMacState = Simulator::ScheduleNow (&LoRaWANMac::SetLoRaWANMacState, this, MAC_IDLE); 
  }
  else {
    NS_LOG_ERROR (this << " MAC state incorrect " << m_LoRaWANMacState);
...
LoRaWANMac::CheckPhyPreamble ()
{
  ...
  if (m_phy->preambleDetected ()) {
    ...
    if (m_LoRaWANMacState == MAC_BEACON )
    {
      m_MissedBeaconCnt++; 
    }
...
LoRaWANMac::StartAckTimeoutTimer ()
{
  ...
  NS_LOG_DEBUG (this << " Starting ACK_TIMEOUT of " << ACK_TIMEOUT << " + " << m_ackTimeOutRandomVariable << " = " << ackTimeout);
...
LoRaWANMac::LoRaWANMacRDC::LoRaWANMacRDC (void) {
  // init sub bands, EU868 CHANGED AU915
  LoRaWANSubBand g0 = {1, 27, Time (), Time ()}; // g(Note 7), 14dBm?
  LoRaWANSubBand g1 = {1, 27, Time (), Time ()}; // 1%
...
LoRaWANMac::LoRaWANMacRDC::IsSubBandAvailable (uint8_t subBandIndex) const
{
  ...
  if (m_subBands[subBandIndex].timeoff.GetMicroSeconds() == 0) // when sending for the first time on this sub band timeoff will be zero, so then the sub band is always available
    return true;
  ...
  NS_LOG_DEBUG (this << " Is " << simTime << " greater than the sum of " << m_subBands[subBandIndex].LastTxFinishedTimestamp << " AND " << m_subBands[subBandIndex].timeoff << "(subbandindex=" << (uint32_t)subBandIndex << "): " << result);
  return result;
}
...
LoRaWANMac::LoRaWANMacRDC::ScheduleSubBandTimer (Ptr<LoRaWANMac> macObj, uint8_t subBandIndex)
{
  ...
  if (!m_subBandTimers[subBandIndex].IsRunning ()) {
    ...
    NS_ASSERT (delay.GetMicroSeconds() > 0); // if equal to zero, then ns3 will get stuck in a loop checking whether the band available ...
...
LoRaWANMac::LoRaWANMacRDC::UpdateRDCTimerForSubBand (uint8_t subBandIndex, Time airTime)
{
  ...
  NS_LOG_LOGIC (this << " updated RDC for subBand " << (uint16_t)subBandIndex << ": time now is: " << Simulator::Now () << ", time finished sending is: "
                     << LastTxFinishedTimestamp << ", meaning a timeoff time of "
                     << timeoff << ", so next usable time is :" << LastTxFinishedTimestamp + timeoff);
}
